...
clean_mega_menu = """<div id="services-mega-panel" style="display:none; position:fixed; top:104px; left:0; right:0; z-index:9999;">
        <div style="background:rgba(8,8,8,0.98); backdrop-filter:blur(24px); -webkit-backdrop-filter:blur(24px); border-bottom:1px solid rgba(255,255,255,0.07); box-shadow:0 20px 60px rgba(0,0,0,0.85);">
            ...
            <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap: 40px 60px;">
                <!-- Left 50% -->
...
for file_path in glob.glob('c:/Elevix Digital/*.html'):
    ...
    if match:
        new_file_content = file_content.replace(match.group(1), clean_mega_menu)
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_file_content)
        print(f"Fixed {file_path}")
    else:
        # Some files might have the old script tag formatted differently
        # Let's try matching up to </nav> minus the script tag
        print(f"Pattern not found in {file_path}")

print("Done completely fixing mega menu layout.")
