...
# 2. Load CTA Banner
with open('c:/Elevix Digital/scratch_cta.html', 'r', encoding='utf-8') as f:
    cta_banner = f.read()
...
# 4. Replace Hero Inner Content
# Find the start of the inner container: '<div class="relative z-10 w-full'
# And replace everything until </section>
start_idx = text.find('<div class="relative z-10 w-full')
end_idx = text.find('</section>', start_idx)

if start_idx != -1 and end_idx != -1:
    text = text[:start_idx] + new_hero_inner + text[end_idx+10:]
else:
    print("Could not find hero inner content to replace.")

# 5. Insert CTA Banner before Footer
# Check if CTA is already there (to avoid duplicates)
if 'READY TO AUTOMATE THE IMPOSSIBLE?' not in text:
    footer_idx = text.find('<footer')
    if footer_idx != -1:
        text = text[:footer_idx] + "\n\n" + cta_banner + "\n\n    " + text[footer_idx:]
    else:
        print("Could not find footer to insert CTA.")

# 6. Save careers.html
with open('c:/Elevix Digital/careers.html', 'w', encoding='utf-8') as f:
    f.write(text)

print("Successfully updated careers hero and added CTA banner!")
