$lines = Get-Content "our-story.html"
$startIdx = -1
$endIdx = -1
for ($i = 0; $i -lt $lines.Length; $i++) {
    if ($lines[$i] -match "<!-- Hero Section -->") { $startIdx = $i }
    if ($lines[$i] -match "<!-- Footer -->") { $endIdx = $i }
}
...
$finalLines = @()
for ($i = 0; $i -lt $startIdx; $i++) { $finalLines += $lines[$i] }
foreach ($l in $newLines) { $finalLines += $l }
for ($i = $endIdx; $i -lt $lines.Length; $i++) { $finalLines += $lines[$i] }

Set-Content "our-story.html" $finalLines -Encoding UTF8
